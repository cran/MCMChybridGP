hybrid.explore <-
function (f, X0, ..., y0 = NULL, n = 200, L = 1000, delta = 0.003,
          nchains = 5, T.mult = 1.5, lb = NULL, ub = NULL,
          maxleap = 0.95, finetune = FALSE,
          Tinclude = nchains, nswaps = choose(nchains, 2),
          graph = FALSE)
{
    negInf <- function(X)
        ...
        for (i in 2:n0)
            for (j in 1:(i - 1))
                if (sum(abs(X[i, ] - X[j, ])) == 0)
                    stop(paste("Duplicate rows (", i, " & ", j, ") in X not allowed", sep = ""))
    ...
    rnd <- function(x) round(10^digits * x) / 10^digits
    ...
    if (graph) {
        if (ncol(X) <= 2) nrefresh.graph <- Inf
        else nrefresh.graph <- ceiling(10 / nchains)
    ...
    if (!is.null(y0)) {
        ...
        for (j in 1:n0) {
            ...
            try({
                ...
                if (OUTSIDE_BOUNDS)
                    stop("X0 violates bounds supplied")
                date1 <- date()
                y[j] <- fn(X[j, ])
                date2 <- date()
                functionOK <- TRUE
    ...
    repeat {
        ...
        if (nchains >= 2) {
            for (reps in 1:nswaps)
                for (index1 in sample(1:nchains))
                    for (index2 in sample((1:nchains)[-index1])) {
                        if (T.mult == 1)
                            Tindex <- 1:nchains
                        else
        ...
        for (chain in 1:nchains) {
            ...
            repeat {
                ...
                try({
                    xp_Early <- Leap(
                        ...
                        L         = L,
                        delta     = delta,
                        Sigma_inv = GPfit$Sigma.inv,
                        dsq_eta   = params,
                        T         = Temp[chain],
                        maxsig    = sqrt(lsq) * maxleap,
                        lb        = low,
                        ub        = upp
                    )
                    xOK <- TRUE
            ...
            if (BOUNDS) {
                for (i in 1:d) {
                    if (x.P[i] < lb[i]) OUTSIDE_BOUNDS <- TRUE
                    if (x.P[i] > ub[i]) OUTSIDE_BOUNDS <- TRUE
                }
            }
            ...
            if (out$Early < 0) {
                ...
                if (corf_x.P > maxleap)
                    stop(paste(
                        "Caution: Leapfrog gave sigmaf =",
                        out$Early / sqrt(lsq),
                        "when sigmaf(x)/lambda =",
                        corf_x.P
                    ))
            }
            ...
            if (graph) {
                ...
                if (!OUTSIDE_BOUNDS)
                    lines(
                        c(x.C[keyVars[1]], x.P[keyVars[1]]),
                        c(x.C[keyVars[2]], x.P[keyVars[2]]),
                        lwd = 0.2, lty = 2, col = clr
                    )
            }

            if (OUTSIDE_BOUNDS) {
                message(paste("T =", Temp[chain], ": Ignore point out of bounds"))
                fx.P <- alpha <- -Inf
            } else {
                date1 <- date()
                fx.P <- fn(x.P)
            ...
            if (log(runif(1)) < alpha) {
                ...
                if (path <= Tinclude) {
                    ...
                    if (nrow(Xfinal) %% floor(n / 10) == 0)
                        message(paste(
                            "hybrid.explore:",
                            round(100 * nrow(Xfinal) / n),
                            "% Complete with",
                            round(100 * mean(acceptance), 1),
            ...
            if (is.null(acceptance))
                ...
                mean.acceptance <- mean(
                    acceptance[ceiling(length(acceptance) / 2):length(acceptance)]
                )

            if (!is.null(Xfinal))
                if (nrow(Xfinal) >= n) break
        ...
        if (graph) {
            if (refresh_count < nrefresh.graph) {
                ...
                refresh_count <- 1
                key.indx <- key.indx + 1
                if (key.indx > nrow(KEY.VARS))
                    key.indx <- 1
                keyVars <- KEY.VARS[key.indx, ]
                graph.template(X, key.vars = keyVars, lb, ub)
                title(c("", "", "", "Exploratory phase"),
                      adj = 0, cex.main = 0.8)
                points(X[, keyVars[1]], X[, keyVars[2]],
                       pch = 20, col = "grey")
                points(Xfinal[, keyVars[1]], Xfinal[, keyVars[2]],
                       pch = 20)
            }
        }
